...
describe("Create Machine Component", () => {
  beforeEach(() => {
    cy.intercept("GET", "http://localhost:8080/items", {fixture: "items_2.json",}).as("getItems")
    cy.intercept("GET", "http://localhost:8080/machines/type", {fixture: "types.json",}).as("getTypes")
    cy.visit("http://localhost:3000/createMachine")
  })
  ...
  it("AC3: Create an Machine with the form and submit", () => {

      cy.intercept("POST", "http://localhost:8080/machine", {body: 1}).as("postMachine")
      cy.wait('@getItems').wait('@getTypes')
      cy.get('[data-cy="name"]').type("Assembling machine 2");
      ...
      cy.wait('@postMachine').then(({request}) => {
